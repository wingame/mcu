...
; NOTE:code page 0090h,3c60h were accessed by program, IT'S REALLY CONFUSE ME.
; TX0 RX0 was routed to port pin p0.4 p0.5, 这里可能是用于串口通信，因此注释掉是不会影响程序
; mov	P0SKIP,#0cfh
...
; 60h store 3830h/38b0h/3870h according to value of 3810h(0/1/2)
...
; 电压保护
; 从62h中读取索引值，从以下数组取一基础电压值(base_volt)0,23h,26h,29h,2ch,2eh
; 开机电压		burden_volt			power_fail_volt
; v<9V			base_volt*2	5.1v	(base_volt+4)*2 5.7v
; 9v< v <13v	base_volt*3	7.7v	(base_volt+4)*3 8.6v
; v>13v			base_volt*4	10.3v	(base_volt+4)*4 11.5v
; 系统电压低于power_fail_volt值时,系统将不受控，并且红灯亮
; 负载电低于burden_volt时，马达将会间歇停顿。
...
temp_uart0_1		equ	30h
current_pwm			equ	35h

; store byte of 0x3840+5
; 低于此值，马达将会间歇停顿
burden_volt		equ	40h
temper				equ	41h
temp_temper			equ	42h
temper_fail_time	equ	43h
burden_volt_fail_time equ 45h
temp_sysvol			equ	51h
t3count_h			equ	47h
t3count_x			equ	48h
...
;db 56h,32h,2eh,30h,30h,5fh,30h,39h,30h,33h,31h,30h,61h,5fh,55h,20h		;V2.00_090310a_U 
db "V2.00_090310a_U "
X0243:
	clr		ea		; 0243   c2 af      B/
	clr		a		; 0245   e4         d
...
X1c38:
	...
	clr		startup_stage		; 1c3d   c2 10      B.
	clr		22h.1		; 1c3f   c2 11      B.
	; 25h.6 好像在需要在另外一边配合使用。正常系统中永远clr
	clr		25h.6		; 1c41   c2 2e      B.
	clr		is_braking		; 1c43   c2 1e      B.
	mov		5ch,#0		; 1c45   75 5c 00   u\.
	mov		temper_fail_time,#0		; 1c48   75 43 00   uC.
	mov		burden_volt_fail_time,#0		; 1c4b   75 45 00   uE.
...
X1c69:
	mov		temp_uart0_1,#4		; 1c69   75 30 04   u0.
...
X1d06:	clr	ri		; 1d06   c2 98      B.
	...
	clr	c		; 1d0a   c3         C
	subb	a,temp_uart0_1		; 1d0b   95 30      .0
	mov	r0,a		; 1d0d   f8         x
	mov	@r0,sbuf	; 1d0e   a6 99      &.
	djnz	temp_uart0_1,X1d16	; 1d10   d5 30 03   U0.
	mov	temp_uart0_1,#4		; 1d13   75 30 04   u0.
...
X1df5:
; 做了很多无谓的比较以后，
; 3800开始的6bytes放到 rcp_max,rcp_min,rcp_nutral中
; 3800~39ff的内容放到xram
...
X21d2:
	ret					; 21d2   22         "
;
X21d3:	clr	exchange_dir		; 21d3   c2 2d      B-
	clr	c		; 21d5   c3         C
...
X246b:	mov	5dh,#90h	; 246b   75 5d 90   u].
	ret			; 246e   22         "
...
; Vdd(3.3v) is reference voltage, branch resistor is 4.7k/1k
...
;
X24af:
;	orl	a,#51h		; 24af   44 51      DQ
;	mov	r2,#0a3h	; 24b1   7a a3      z#
;	mov	r7,a		; 24b3   ff         .
...
X252e:
	ret			; 252e   22         "
;
X252f:	mov	a,accelate_factor		; 252f   e5 2e      e.
	subb	a,5dh		; 2531   95 5d      .]
	jnc	X2557		; 2533   50 22      P"
	mov	a,idle_4ah		; 2535   e5 4a      eJ
	jnz	X2544		; 2537   70 0b      p.
	clr	c		; 2539   c3         C
	mov	a,idle_49h		; 253a   e5 49      eI
	subb	a,#50h		; 253c   94 50      .P
	jc	X2556		; 253e   40 16      @.
	mov	idle_49h,a		; 2540   f5 49      uI
	sjmp	X254f		; 2542   80 0b      ..
;
X2544:	clr	c		; 2544   c3         C
	mov	a,idle_49h		; 2545   e5 49      eI
	subb	a,#50h		; 2547   94 50      .P
	mov	idle_49h,a		; 2549   f5 49      uI
	jnc	X254f		; 254b   50 02      P.
	dec	idle_4ah		; 254d   15 4a      .J
X254f:	mov	a,accelate_factor		; 254f   e5 2e      e.
	clr	c		; 2551   c3         C
	add	a,54h		; 2552   25 54      %T
	mov	accelate_factor,a		; 2554   f5 2e      u.
X2556:	ret			; 2556   22         "
;
X2557:	mov	accelate_factor,5dh		; 2557   85 5d 2e   .].
	mov	idle_4ah,#0		; 255a   75 4a 00   uJ.
	mov	idle_49h,#0		; 255d   75 49 00   uI.
	ret			; 2560   22         "
;
X2561_all_fet_off:
...
;
X257d:
	mov		a,idle_4ah		; 257d   e5 4a      eJ
	jnz		X258c		; 257f   70 0b      p.
	clr		c		; 2581   c3         C
	mov		a,idle_49h		; 2582   e5 49      eI
	subb	a,#5		; 2584   94 05      ..
	jc		X25a2		; 2586   40 1a      @.
	mov		idle_49h,a		; 2588   f5 49      uI
	sjmp	X2597		; 258a   80 0b      ..
;
X258c:	clr	c		; 258c   c3         C
	mov	a,idle_49h		; 258d   e5 49      eI
	subb	a,#5		; 258f   94 05      ..
	mov	idle_49h,a		; 2591   f5 49      uI
	jnc	X2597		; 2593   50 02      P.
	dec	idle_4ah		; 2595   15 4a      .J
...
x25a3_control_start:
	...
	mov		idle_4ah,#0		; 25bd   75 4a 00   uJ.
	mov		idle_49h,#0		; 25c0   75 49 00   uI.
...
x25cc_start_loop:
	...
	jnb		is_braking,X25da	; 25d4   30 1e 03   0..
...
X25f4:
	...
	jnb		is_braking,X2605	; 25ff   30 1e 03   0..
	ljmp	X26aa		; 2602   02 26 aa   .&*
;
X2605:	ljmp	x26ea_brake_motor		; 2605   02 26 ea   .&j
;
X2608:
	jnb		21h.6,X2611	; 2608   30 0e 06   0..
	jnb		is_braking,X2611	; 260b   30 1e 03   0..
	ljmp	X26aa		; 260e   02 26 aa   .&*
;
X2611:	jb	startup_stage,X2617	; 2611   20 10 03    ..
...
X261d:
	...
	; p0.2 feedback neutral, exam motor is running or not.
	...
	mov		r3,#5		; 262c   7b 05      {.
X262e:
	clr		AD0INT		; 262e   c2 ed      Bm
	setb	AD0BUSY		; 2630   d2 ec      Rl
	jnb		AD0INT,$	; 2632   30 ed fd   0m}
...
;
X264c:	jnb	is_braking,X26b6	; 264c   30 1e 67   0.g
	jnb	21h.6,X26b6	; 264f   30 0e 64   0.d
	ljmp	X26aa		; 2652   02 26 aa   .&*
;
X2655:	cjne	a,#21h,X2661	; 2655   b4 21 09   4!.
	jb	is_braking,X265e	; 2658   20 1e 03    ..
	ljmp	X26a8		; 265b   02 26 a8   .&(
;
X265e:	ljmp	x26ea_brake_motor		; 265e   02 26 ea   .&j
;
X2661:	mov	run_braking_cnt,#0		; 2661   75 53 00   uS.
...
;
X2695:	mov	dptr,#X28f2	; 2695   90 28 f2   .(r
	mov	a,61h		; 2698   e5 61      ea
X269a:	movc	a,@a+dptr	; 269a   93         .
	mov	r2,a		; 269b   fa         z
	lcall	x274c_brake_pwm_r2		; 269c   12 27 4c   .'L
	jnb	rcp_t0_ready,X26a5	; 269f   30 00 03   0..
	lcall	x2e69_verify_rcp		; 26a2   12 2e 69   ..i
X26a5:	ljmp	x25cc_start_loop		; 26a5   02 25 cc   .%L
;
X26a8:	clr	dir_reverse		; 26a8   c2 0a      B.
X26aa:	lcall	x2816_cfg_according_params		; 26aa   12 28 16   .(.
	mov	OSCICN,#0c3h	; 26ad   75 b2 c3   u2C
	ljmp	X27f4		; 26b0   02 27 f4   .'t
;
X26b3:	lcall	x274c_brake_pwm_r2		; 26b3   12 27 4c   .'L
X26b6:	lcall	x274c_brake_pwm_r2		; 26b6   12 27 4c   .'L
	jb	rcp_t0_ready,X26bf	; 26b9   20 00 03    ..
	ljmp	x25cc_start_loop		; 26bc   02 25 cc   .%L
;
X26bf:	lcall	x2e69_verify_rcp		; 26bf   12 2e 69   ..i
	...
	subb	a,#5		; 26d4   94 05      ..
	jc	X26e7		; 26d6   40 0f      @.
	jb	startup_stage,X26df	; 26d8   20 10 04    ..
	setb	startup_stage		; 26db   d2 10      R.
	sjmp	X26e4		; 26dd   80 05      ..
;
X26df:	jnb	25h.6,X26b6	; 26df   30 2e d4   0.T
	setb	22h.1		; 26e2   d2 11      R.
X26e4:	mov	run_braking_cnt,#0		; 26e4   75 53 00   uS.
X26e7:	ljmp	x25cc_start_loop		; 26e7   02 25 cc   .%L
;
...
X27a2:
	...
	mov		XBR1,#41h	; 27ab   75 e2 41   ubA
...
X27eb:
	...
	jb		startup_stage,X27f4	; 27ee   20 10 03    ..
...
x2816_cfg_according_params:
	...
	mov		idle_4ah,a		; 2824   f5 4a      uJ
	mov		idle_49h,a		; 2826   f5 49      uI
	...
	; 60h is never be zero. so 21h.6 always be set
...
;
X292a:
	sjmp	X2930		; 292a   80 04      ..
	sjmp	X2938		; 292c   80 0a      ..
	sjmp	X2940		; 292e   80 10      ..
X2930:
...
X2948:
	mov		r0,#4		; 2948   78 04      x.
X294a:
	mov		a,TMR2H		; 294a   e5 cd      eM
...
X29a7:
	lcall	x2808_t2_reset		; 29a7   12 28 08   .(.
X29aa:
	mov		r0,#3		; 29aa   78 03      x.
X29ac:
	mov		a,TMR2H		; 29ac   e5 cd      eM
...
X2a5c:	lcall	x2808_t2_reset		; 2a5c   12 28 08   .(.
	mov	idle_4ah,#0		; 2a5f   75 4a 00   uJ.
	mov	idle_49h,#0		; 2a62   75 49 00   uI.
...
;
X2a7b:
; 有惯性的启动马达， RESTART MOTOR
	; LG = 1
	mov	ADC0GTH,#0		; 2a7b   75 c4 00   uD.
...
X2aa1:
	mov		a,b		; 2aa1   e5 f0      ep
	clr	c		; 2aa3   c3         C
	add	a,idle_49h		; 2aa4   25 49      %I
	mov	idle_49h,a		; 2aa6   f5 49      uI
	jnc	X2aac		; 2aa8   50 02      P.
	inc	idle_4ah		; 2aaa   05 4a      .J

X2aac:	mov	a,temper_fail_time		; 2aac   e5 43      eC
	add	a,t3count_x		; 2aae   25 48      %H
	mov	temper_fail_time,a		; 2ab0   f5 43      uC
	mov	a,burden_volt_fail_time		; 2ab2   e5 45      eE
	add	a,t3count_x		; 2ab4   25 48      %H
	mov	burden_volt_fail_time,a		; 2ab6   f5 45      uE
	...
	inc	burden_volt_fail_time		; 2ad6   05 45      .E
	inc	state_timeout		; 2ad8   05 44      .D
	inc	idle_4ah		; 2ada   05 4a      .J
...
; startup_count 中初始值为40h
	mov		idle_4ah,#0		; 2ae5   75 4a 00   uJ.
	mov		idle_49h,#0		; 2ae8   75 49 00   uI.
...
;
X2afc:
	mov		a,5fh		; 2afc   e5 5f      e_
	clr		c		; 2afe   c3         C
	subb	a,#10h		; 2aff   94 10      ..
	jc		X2af8		; 2b01   40 f5      @u
X2b03:
	mov		5fh,a		; 2b03   f5 5f      u_
	clr		c		; 2b05   c3         C
...
;
X2b9e:	lcall	x3213_running_delay		; 2b9e   12 32 13   .2.
...
;
X2bb4:
	lcall	x2cde_feedback_c		; 2bb4   12 2c de   .,^
	; fet_an = 1, 2313 will drive AN FET off
	jb		fet_an,X2bd5	; 2bb7   20 95 1b    ..
X2bba:
	mov		a,TMR2H		; 2bba   e5 cd      eM
	clr		c		; 2bbc   c3         C
	;init 5fh=#0feh at 2aee
...
X2bc4:
	ljmp	X3043		; 2bc4   02 30 43   .0C
;
X2bc7:
	lcall	x2528_check_ready_rcp		; 2bc7   12 25 28   .%(
...
;
X2bd5:
	lcall	x3213_running_delay		; 2bd5   12 32 13   .2.
X2bd8:
	mov		r0,#3		; 2bd8   78 03      x.
X2bda:
	jnb		fet_an,X2bba	; 2bda   30 95 dd   0.]
...
;
X2c15:	lcall	x3213_running_delay		; 2c15   12 32 13   .2.
...
;
X2c4c:	lcall	x3213_running_delay		; 2c4c   12 32 13   .2.
...
;
X2c83:	lcall	x3213_running_delay		; 2c83   12 32 13   .2.
...
;
X2cc3:	lcall	x3213_running_delay		; 2cc3   12 32 13   .2.
...
;
x2ce3_feedback_a:
	mov		AMX0P,#1		; 2ce3   75 bb 01   u;.
X2ce6:
	mov		AMX0N,#2		; 2ce6   75 ba 02   u:.
; ADC0CF[AD0SC(4:0):AD0LJST:-:-]
; AD0SC = 00111
; AD0LJST = 1 left justified
	mov		ADC0CF,#3ch	; 2ce9   75 bc 3c   u<<
; AD0EN = 1
	mov		ADC0CN,#80h	; 2cec   75 e8 80   uh.
	ret			; 2cef   22         "

; temper > #4fh           , temper_fail_time=#0
; temper < #4fh & temper_fail_time>#64h, setb stay_idle
x2cf0_fetch_temper:
	clr	stay_idle		; 2cf0   c2 01      B.
; 6ah 0 使能温度检测
...
; init temper_fail_time=0 at 1c48h
...
x2d74_exam_volt:
	...
	jc	X2d8b		; 2d7e   40 0b      @.
	mov	a,burden_volt_fail_time		; 2d80   e5 45      eE
	jz	X2d94		; 2d82   60 10      `.
	dec	burden_volt_fail_time		; 2d84   15 45      .E
...
X2d8b:
	clr		c		; 2d8b   c3         C
	mov		a,burden_volt_fail_time		; 2d8c   e5 45      eE
	; 负载电压一直维持在较低值到0f8h时马达停止转动
...
;
X2d95:
	mov		a,burden_volt		; 2d95   e5 40      e@
...
; init burden_volt_fail_time=0 at 1c4bh
	mov	a,burden_volt_fail_time		; 2dc5   e5 45      eE
	clr	c		; 2dc7   c3         C
	subb	a,#3ch		; 2dc8   94 3c      .<
	jc	X2dd1		; 2dca   40 05      @.
	setb	adc_busy		; 2dcc   d2 33      R3
	mov	burden_volt_fail_time,#0		; 2dce   75 45 00   uE.
X2dd1:	ret			; 2dd1   22         "
;
X2dd2:	mov	burden_volt_fail_time,#0		; 2dd2   75 45 00   uE.
	ret			; 2dd5   22         "
;
X2dd6:	mov	temp_sysvol,ADC0H	; 2dd6   85 be 51   .>Q
...
X2e23:
	ret			; 2e23   22         "
;
X2e24:	jb	exchange_dir,X2e23	; 2e24   20 2d fc    -|
...
;
x2e34_zero_cross_i0:	jnb	exchange_dir,X2e2f	; 2e34   30 2d f8   0-x
...
X2e5e:
	ret			; 2e5e   22         "
;
X2e5f:	jb	exchange_dir,X2e5e	; 2e5f   20 2d fc    -|
	cpl	zc_point		; 2e62   b2 09      2.
	ret			; 2e64   22         "
;
X2e65:
	setb	fb_adc_timeout		; 2e65   d2 05      R.
	ret			; 2e67   22         "
;
X2e68:	ret			; 2e68   22         "
;
...
X2ebd:
	...
	cpl	a		; 2ec6   f4         t
	mov	r0,a		; 2ec7   f8         x
	jnb	startup_stage,X2ece	; 2ec8   30 10 03   0..
	; 3ch=#0eh
...
X2efd:	mov	r0,a		; 2efd   f8         x
	inc	r1		; 2efe   09         .
X2eff:
	djnz	37h,X2eeb	; 2eff   d5 37 e9   U7i
	mov		a,r1		; 2f02   e9         i
	mov		r2,a		; 2f03   fa         z
...
;
X2f36:
	cjne	a,5ch,X2f3b	; 2f36   b5 5c 02   5\.
	sjmp	X2f41		; 2f39   80 06      ..
;
X2f3b:
	mov		5ch,a		; 2f3b   f5 5c      u\ 
	mov		r2,2fh		; 2f3d   aa 2f      */
	sjmp	X2f50		; 2f3f   80 0f      ..
;
X2f41:
	mov		2fh,r2		; 2f41   8a 2f      ./
...
X2f50:
	mov		a,motor_status		; 2f50   e5 59      eY
	cjne	a,#1,X2f57	; 2f52   b4 01 02   4..
	sjmp	X2f99		; 2f55   80 42      .B
;
X2f57:
	cjne	a,#2,X2f5c	; 2f57   b4 02 02   4..
	sjmp	X2f6d		; 2f5a   80 11      ..
;
X2f5c:
	cjne	a,#11h,X2f61	; 2f5c   b4 11 02   4..
	sjmp	X2f99		; 2f5f   80 38      .8
;
X2f61:
	cjne	a,#22h,X2f66	; 2f61   b4 22 02   4".
...
;
X2f99:
	clr		dir_reverse		; 2f99   c2 0a      B.
X2f9b:
	jnb		forwarding,X2fd5	; 2f9b   30 03 37   0.7
	clr		c		; 2f9e   c3         C
	mov		a,r2		; 2f9f   ea         j
...
X2fcc:
	mov		accelate_factor,a		; 2fcc   f5 2e      u.
X2fce:
	mov		PCA0CPH0,r2		; 2fce   8a fc      .|
	setb	red_led		; 2fd0   d2 94      R.
	clr		green_led		; 2fd2   c2 93      B.
	ret			; 2fd4   22         "
;
X2fd5:
	clr		green_led		; 2fd5   c2 93      B.
...
;
X303b:
	inc	startup_count		; 303b   05 4f      .O
X303d:
	jnb	TMR2CN.7,$	; 303d   30 cf fd   0O}
...
X3077:
	...
	setb	is_braking		; 3080   d2 1e      R.
	sjmp	X30cb		; 3082   80 47      .G
;
X3084:
	jnb		dir_reverse,X30c0	; 3084   30 0a 39   0.9
	jnb		first_3state,X309d	; 3087   30 2f 13   0/.
	setb	is_braking		; 308a   d2 1e      R.
	clr	startup_stage		; 308c   c2 10      B.
	clr	22h.1		; 308e   c2 11      B.
	sjmp	X309d		; 3090   80 0b      ..
;
X3092:	jnb	first_3state,X309d	; 3092   30 2f 08   0/.
	clr	is_braking		; 3095   c2 1e      B.
	clr	startup_stage		; 3097   c2 10      B.
...
X30a3:
	...
	setb	is_braking		; 30b8   d2 1e      R.
	clr	startup_stage		; 30ba   c2 10      B.
	clr	22h.1		; 30bc   c2 11      B.
	sjmp	X30cb		; 30be   80 0b      ..
;
X30c0:	jnb	first_3state,X30cb	; 30c0   30 2f 08   0/.
	clr	is_braking		; 30c3   c2 1e      B.
	clr	startup_stage		; 30c5   c2 10      B.
	clr	22h.1		; 30c7   c2 11      B.
	clr	25h.6		; 30c9   c2 2e      B.
X30cb:	mov	a,state_index		; 30cb   e5 29      e)
...
x316c_flash_green:
	...
	mov		P1SKIP,#0fdh	; 3178   75 d5 fd   uU}
...
x31a4_long_light_green:
	...
	mov		P1SKIP,#0fdh	; 31ae   75 d5 fd   uU}			; p1.1 cn
...
x3213_running_delay:
	lcall	X3219		; 3213   12 32 19   .2.
	lcall	X3219		; 3216   12 32 19   .2.
X3219:
	lcall	X3228		; 3219   12 32 28   .2(
...
X322b:
	ljmp	X322e		; 322b   02 32 2e   .2.
X322e:
	ljmp	X3231		; 322e   02 32 31   .21
X3231:
	ljmp	X3234		; 3231   02 32 34   .24
X3234:
	ljmp	X3237		; 3234   02 32 37   .27
;
X3237:	ret			; 3237   22         "
;
...
X323e:	jb	adc_busy,X3272	; 323e   20 33 31    31
	...
	mov	idle_4ah,#0		; 3246   75 4a 00   uJ.
	mov	idle_49h,#0		; 3249   75 49 00   uI.
	ret			; 324c   22         "
;
X324d:	mov	a,idle_4ah		; 324d   e5 4a      eJ
	jnz	X325c		; 324f   70 0b      p.
	clr	c		; 3251   c3         C
	mov	a,idle_49h		; 3252   e5 49      eI
	subb	a,#50h		; 3254   94 50      .P
	jc	X3272		; 3256   40 1a      @.
	mov	idle_49h,a		; 3258   f5 49      uI
	sjmp	X3267		; 325a   80 0b      ..
;
X325c:	clr	c		; 325c   c3         C
	mov	a,idle_49h		; 325d   e5 49      eI
	subb	a,#50h		; 325f   94 50      .P
	mov	idle_49h,a		; 3261   f5 49      uI
	jnc	X3267		; 3263   50 02      P.
	dec	idle_4ah		; 3265   15 4a      .J
...
X328c:
	; GT=4
	mov		ADC0GTH,#1		; 328c   75 c4 01   uD.
...
;
X32fc:	lcall	x2ce3_feedback_a		; 32fc   12 2c e3   .,c
...
X3304:
	...
	jc	X3311		; 3309   40 06      @.
	; seems program is never reach here
...
X3349:	lcall	x2a90_t3_sum		; 3349   12 2a 90   .*.
...
;
x3466_zc_happen:
	lcall	x2808_t2_reset		; 3466   12 28 08   .(.
	mov		a,r4		; 3469   ec         l
	clr		c		; 346a   c3         C
	addc	a,r6		; 346b   3e         >
	mov		r0,a		; 346c   f8         x
...
X3ce5:	mov	dpl,r2		; 3ce5   8a 82      ..
	...
	mov	r0,a		; 3cec   f8         x
	cjne	r2,#low(X3680),X3cf5	; 3ced   ba 80 05   :..
	cjne	r3,#high(X3680),X3cf5	; 3cf0   bb 36 02   ;6.
...
;
x3d6b_pin_init:
	mov		P0MDIN,#70h	; 3d6b   75 f1 70   uqp
