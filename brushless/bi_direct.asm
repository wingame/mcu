...
.def	flag1			= r24
	.equ	tcnt0_h_ovf	= 2
	.equ	last_direct = 3		; last direction 0 forward, 1 reverse
...
.equ	RCP_HIGH				= 14400 * (F_CPU/8)		; 1800us
;.equ	RCP_HIGH				= 14000 * (F_CPU/8)		; 1800us
.equ	RCP_LOW					= 8800 * (F_CPU/8)		; 1100us
.equ	RCP_STOP_AREA			= 500 * (F_CPU/8)
.equ	RCP_NUETRAL_MARGIN		= 1600 * (F_CPU/8)		; 2000us
...
.macro	motor_stop
	andi	flag1,0b11111100
.endm
.equ		STOP	= 0
.macro	motor_forward
	sbr		flag1,0b00000001
	cbr		flag1,0b00000010
.endm
.equ		FORWARD	= 1
.macro	motor_brake
	ori		flag1,0b00000011
.endm
.equ		BRAKE	= 3
.macro	motor_reverse
	sbr		flag1,0b00000010
	cbr		flag1,0b00000001
.endm
.equ		REVERSE	= 2
.macro motor_dir
	ldi		@0,0b00000011
...
mem_rcp_low_range:	.byte	2
mem_rcp_high_range:	.byte	2
...
wait_rcp_neutral:
	rcall	rcp_ready
	movw	r_temp1,rcp_l
	subi	r_temp1,low(RCP_LOW+RCP_NUETRAL_MARGIN)
	sbci	r_temp2,high(RCP_LOW+RCP_NUETRAL_MARGIN)
	subi	r_temp1,low(RCP_HIGH-RCP_LOW-RCP_NUETRAL_MARGIN*2)
	subi	r_temp2,high(RCP_HIGH-RCP_LOW-RCP_NUETRAL_MARGIN*2)
	brcc	wait_rcp_neutral
	dec		r_temp3
	brne	wait_rcp_neutral
	sts		mem_rcp_neutral,rcp_l
	sts		mem_rcp_neutral+1,rcp_h
	
	movw	r_temp1,rcp_l
	subi	r_temp1,low(RCP_LOW)
	subi	r_temp2,high(RCP_LOW)
	sts		mem_rcp_low_range,r_temp1
	sts		mem_rcp_low_range+1,r_temp2

	ldi		r_temp1,low(RCP_HIGH)
	ldi		r_temp2,high(RCP_HIGH)
	sub		r_temp1,rcp_l
	sbc		r_temp2,rcp_h
	sts		mem_rcp_high_range,r_temp1
	sts		mem_rcp_high_range+1,r_temp2
...
ra_1:
.endif
	; 无论正或者反，直接从state 0 开始可以顺利接上，这是什么道理?
;	ldi		r_temp1,0
;	mov		state_index,r_temp1
	clr		state_index
	rcall	next_state
...
wait_rcp_available:
	mov		rcp_error_cnt,r_temp1
	brtc	wait_rcp_available
	clt
	dec		r_temp2
	brne	wait_rcp_available
;	rjmp	rcp_resume
	rjmp	reset
;------------------------------------------------------------------------------------
t2ovfl_int:
...
evaluate_rcp:
	...
	cpc		rcp_h,r_temp2
	brcc	eval_rcp_high
	subi	r_temp3,low(RCP_STOP_AREA)
	sbci	r_temp4,high(RCP_STOP_AREA)
	cp		rcp_l,r_temp3
	cpc		rcp_h,r_temp4
	brcs	eval_rcp_low
	; rcp neutral
...
eval_rcp_high:
	motor_forward
	movw	r_temp1,rcp_l
	subi	r_temp1,low(RCP_HIGH)
	sbci	r_temp2,high(RCP_HIGH)
	brcs	calc_pwm_2
;debug **********************************************
...
eval_rcp_low:
	...
	lds		r_temp1,mem_rcp_low_range
	lds		r_temp2,mem_rcp_low_range+1
	sts		mem_b_l,r_temp1
	sts		mem_b_h,r_temp2
	rcall	x_function
	rjmp	approach_pwm
calc_pwm_2:
	...
	lds		r_temp1,mem_rcp_high_range
	lds		r_temp2,mem_rcp_high_range+1
	sts		mem_b_l,r_temp1
	sts		mem_b_h,r_temp2
	rcall	x_function
	rjmp	approach_pwm
